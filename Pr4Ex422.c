...
int comparacioapartirdindexniu(const void * a, const void * b);
...
int main(){
    FILE *dades;
    Alu *alumnes, **indexmitjanes, **indexniu, *indexBuscat;
    int n, i, ll, num_lin = 0, lrg = 0, NIUBuscat;
    ...
    if ((indexniu = (Alu **)malloc(sizeof(Alu *)*num_lin)) == NULL){
        printf("Error: no hi ha espai suficient en la memoria.");
        return 2;
    }
    ...
    qsort(indexniu, num_lin, sizeof(Alu *), comparacioapartirdindexniu);
    fclose(dades);
    ...
    if (indexBuscat == NULL){
        ...
        printf(" |%6.2f",indexBuscat->notes[4]);
        printf("\n");
    }
    // Alliberem memoria
    free(alumnes);
...
// Funcions per imprimir les dades
void imprimirtaula(int num_lin, Alu *alumnes){
    int i,j;
    for(j=0;j<num_lin;j++){
